    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        editButtons.forEach(button => {
            button.addEventListener("click", function () {
                ...
                // Set URL form edit
                editForm.action = `{{ route('admin.penerbit.update', '') }}/${id}`;
            });
        });

        // Tambah handler saat form dikirim
...
function konfirmasiHapus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.action = `{{ route('admin.penerbit.destroy', '') }}/${id}`;
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Fitur pencarian penerbit
        document.getElementById('searchPenerbit').addEventListener('keyup', function() {
            let searchValue = this.value.toLowerCase();
            let rows = document.querySelectorAll('.table tbody tr');

            rows.forEach(row => {
                let penerbitName = row.cells[1].innerText.toLowerCase(); // Ambil nama penerbit dari kolom ke-2
                if (penerbitName.includes(searchValue)) {
                    row.style.display = "";
                } else {
